...
class AopClient {
	...
	public function alonersaSign($data, $privatekey, $signType = "RSA", $keyfromfile = false) {
		...
		if (!$keyfromfile) {
			$priKey = $privatekey;
			//\app\alipay\sdk\writeLog('alonersaSign() privatekey: ' . $priKey);
			$res = "-----BEGIN RSA PRIVATE KEY-----\n" .
					wordwrap($priKey, 64, "\n", true) .
					"\n-----END RSA PRIVATE KEY-----";
			//\app\alipay\sdk\writeLog('alonersaSign() RSA PRIVATE KEY: ' . "\n" . $res);
		} else {
		...
		if (!$b) {
			//\app\alipay\sdk\writeLog('openssl_sign() ->  false ');
		}
...
//		$logger->log($logData);
	...
	public function pageExecute($request, $httpmethod = "POST") {
		...
		if (strcasecmp($this->fileCharset, $this->postCharset)) {
			...
			// \app\alipay\sdk\writeLog("本地文件字符集编码与表单提交编码不一致，请务必设置成一样，属性名分别为postCharset!");
...
//        \app\alipay\sdk\writeLog('$signType: ' . $signType);

		if ($this->checkEmpty($this->alipayPublicKey)) {

			$pubKey = $this->alipayrsaPublicKey;
			//\app\alipay\sdk\writeLog('alipayrsaPublicKey: ' . $pubKey);
			$res = "-----BEGIN PUBLIC KEY-----\n" .
					wordwrap($pubKey, 64, "\n", true) .
					"\n-----END PUBLIC KEY-----";
			//\app\alipay\sdk\writeLog('alipay PUBLIC KEY: ' . "\n" . $res);
		} else {
			//读取公钥文件
    ...
    static public function verifySign($data, $sign){
        print_r(config('rsa.platform_private_key'));
        print_r(config('rsa.platform_public_key'));die;
