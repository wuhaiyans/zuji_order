...
//			LogApi::info('获取支付的params', ['params'=>$params]);
   ...
   /**
    ...
    public function refundClean(Request $request){
        ...
        try{
            $input = file_get_contents("php://input");
            LogApi::info(__method__.'[财务清算回调退款]回调接收',$input);
			...
			if ($validateParams['code']!=0) {
                LogApi::error(__method__.'[财务清算回调退款]参数校验错误',$validateParams);
				$this->innerErrMsg($validateParams['msg']);
			}
            if ($param['status']!='success'){
                LogApi::error(__method__.'[财务清算回调退款]状态错误',$param);
            }
            ...
            if ($orderCleanInfo['code']) {
				DB::rollback();
                LogApi::error(__method__.'[财务清算回调退款][清算记录]不存在');
                ...
                if (!$success) {
                    ...
                    if ($orderCleanInfo['auth_deduction_status']!=OrderCleaningStatus::depositDeductionStatusUnpayed &&
                        ...
                        if (!$success) {	// 成功
							...
							if( !$b ){
                                DB::rollBack();
                                LogApi::error(__method__.'[财务清算回调退款]业务接口失败OrderCleaning::getBusinessCleanCallback', [$businessParam, $userinfo]);
                                $this->innerErrMsg(__METHOD__."() ".microtime(true).' 退款回调业务接口失败');
							}

                            LogApi::info(__method__.'[财务清算回调退款]业务接口OrderCleaning::getBusinessCleanCallback返回的结果', $success);
							
                        }  else {
							DB::rollBack();
                            LogApi::error(__method__.'[财务清算回调退款]业务回调更新整体清算的状态失败', $orderParam);
					...
					DB::rollBack();
                    $this->innerErrMsg();
                    LogApi::error(__method__.'[财务清算回调退款]退款业务状态更新失败');
                    $this->innerErrMsg(__METHOD__."() ".microtime(true).' 退款业务状态更新失败');
                }

            } else { // 非待退款状态
				DB::rollBack();
                LogApi::error(__method__.'[财务清算回调退款]订单清算退款状态无效');
			...
			DB::rollBack();
            LogApi::error(__method__.'[财务清算回调退款]订单清算退款回调地址异常',  [$e,$param]);
    ...
    public function unFreezeClean(Request $request)
    {
        try{
            $input = file_get_contents("php://input");
            LogApi::info(__method__.'[财务清算回调解除预授权]订单清算退押金回调接口回调参数:',$input);
            $param = json_decode($input,true);
            $rule = [
...
//            $orderCleanInfo = OrderCleaning::getOrderCleanInfo(['clean_no'=>'CA70407132618675']);
            ...
            if (!isset($orderCleanInfo['code']) || $orderCleanInfo['code']) {
                LogApi::error(__method__.'[财务清算回调解除预授权]订单清算记录不存在');
            ...
            if ($orderCleanInfo['auth_unfreeze_status']==OrderCleaningStatus::depositUnfreezeStatusUnpayed){
                    ...
                    if ($orderCleanInfo['auth_deduction_status']!=OrderCleaningStatus::depositDeductionStatusUnpayed &&
                        ...
                        if (!$success) {
                            ...
                            if( !$success ){
                                ...
                                DB::rollBack();
                                LogApi::error(__method__.'[财务清算回调解除预授权]回调业务接口失败OrderCleaning::getBusinessCleanCallback', [$businessParam, $userinfo,$success]);
                                $this->innerErrMsg('押金解押业务回调更新整体清算的状态失败');
                            }

                            LogApi::debug(__method__.'[财务清算回调解除预授权]回调业务接口参数及结果OrderCleaning::getBusinessCleanCallback', [$businessParam, $success]);
                        }  else {

                            DB::rollBack();
                            LogApi::error(__method__.'[财务清算回调解除预授权]业务回调更新整体清算的状态失败', $orderParam);
                            $this->innerErrMsg('押金解押业务回调更新整体清算的状态失败');
                        }
                    }
                } else {
                    DB::rollBack();
                    LogApi::error(__method__.'[财务清算回调解除预授权] 更新订单退押金状态失败');
               ...
               // DB::rollBack();
                LogApi::error(__method__.'[财务清算回调解除预授权]订单清算退款状态无效',$param);
            ...
            DB::rollBack();
            LogApi::error(__method__.'[财务清算回调解除预授权]订单清算退押金回调接口异常 ' ,$e);
    ...
    public function unfreezeAndPayClean(Request $request)
    {
        try{
            $input = file_get_contents("php://input");
            LogApi::info(__method__.'[财务清算回调预授权转支付]订单清算退押金回调接口回调参数:'.$input);
            $param = json_decode($input,true);
            $rule = [
                "status"=>'required',                //类型：String  必有字段  备注：init：初始化；success：成功；failed：失败；finished：完成；closed：关闭； processing：处理中；
                "trade_no"=>'required',                //类型：String  必有字段  备注：支付平台交易码
            ...
            if ($orderCleanInfo['code']) {
                LogApi::error(__method__.'[财务清算回调预授权转支付]订单清算记录不存在');
                $this->innerErrMsg('订单清算记录不存在');
                ...
                if (!$success) {
                    ...
                    if ($orderCleanInfo['refund_status']!=OrderCleaningStatus::refundUnpayed &&
                        ...
                        if (!$success) {
                            ...
                            if( !$success ){//
                                DB::rollBack();
                                LogApi::error(__method__.'[财务清算回调预授权转支付]回调业务失败参数及结果OrderCleaning::getBusinessCleanCallback', [$businessParam,$userinfo,$success]);
                                $this->innerErrMsg('押金转支付回调业务业务失败');
                            }
                            LogApi::info(__method__.'[财务清算回调预授权转支付]调业务接口参数及结果OrderCleaning::getBusinessCleanCallback', [$businessParam,$success]);
                        }  else {
                            DB::rollBack();
                            LogApi::error(__method__.'[财务清算回调预授权转支付]回调更新整体清算的状态失败', $orderParam);
                            $this->innerErrMsg('押金转支付回调更新整体清算的状态失败');
                        }
                    }
                } else {
                    DB::rollBack();
                    LogApi::error(__method__.'[财务清算回调预授权转支付]更新押金转支付的状态失败');
                    $this->innerErrMsg('押金转支付的状态更新失败');
                }

            } else {

                LogApi::error(__method__.'[财务清算回调预授权转支付]订单清算退款状态无效');
            ...
            DB::rollBack();
            LogApi::error(__method__.'[财务清算回调预授权转支付]回调接口异常 ', [$e,$param]);
